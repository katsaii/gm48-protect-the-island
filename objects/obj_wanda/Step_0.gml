...
var xdir = max(clamp(keyboard_direction(vk_left, vk_right) + keyboard_direction(ord("A"), ord("D")), -1, 1) - blast * 1.5, -1);
...
if (blastTimer == -1) {
    if (blast) {
        ...
        with (instance_create_depth(x, y - 10 + random_range(-5, 5), depth, obj_wanda_projectile)) {
            speed = 10;
            direction = (other.xspeed + other.yspeed) + random_range(-2, 2);
            hspeed += other.xspeed;
            vspeed += other.yspeed;
