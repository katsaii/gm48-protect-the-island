...
#macro __INSTANCE_CREATE_DEPTH instance_create_depth
#macro __INSTANCE_CREATE_LAYER instance_create_layer
#macro __INSTANCE_DEACTIVATE_ALL instance_deactivate_all
#macro __INSTANCE_DEACTIVATE_OBJECT instance_deactivate_object
#macro __INSTANCE_DEACTIVATE_REGION instance_deactivate_region
#macro __INSTANCE_DEACTIVATE_LAYER instance_deactivate_layer
#macro __INSTANCE_COPY instance_copy
#macro __INSTANCE_DESTROY instance_destroy
#macro instance_create_depth __instance_create_depth
#macro instance_create_layer __instance_create_layer
#macro instance_deactivate_all __instance_deactivate_all
#macro instance_deactivate_object __instance_deactivate_object
#macro instance_deactivate_region __instance_deactivate_region
#macro instance_deactivate_layer __instance_deactivate_layer
#macro instance_copy __instance_copy
#macro instance_destroy __instance_destroy
...
/// @param {real} obj The object index to check.
function __object_get_diagnostic(_obj) {
...
function __instance_create_depth(_x, _y, _depth, _obj) {
	var error = __object_get_diagnostic(_obj);
	if (error != undefined) {
		show_error(error, false);
		return noone;
	}
	return __INSTANCE_CREATE_DEPTH(_x, _y, _depth, _obj);
}
...
/// @param {real} obj The object index to create an instance of.
function __instance_create_layer(_x, _y, _layer, _obj) {
	var error = __object_get_diagnostic(_obj);
	if (error != undefined) {
		show_error(error, false);
		return noone;
	}
	return __INSTANCE_CREATE_LAYER(_x, _y, _layer, _obj);
}

/// @desc Wrapper for `instance_deactivate_all` which does not deactivate singleton objects.
/// @param {bool} notme Whether to ignore the calling instance.
function __instance_deactivate_all(_notme) {
	var me = id;
	with (all) {
		if (object_is_singleton(object_index) || object_is_system(object_index) || id == me) {
			continue;
		}
		__INSTANCE_DEACTIVATE_OBJECT(id);
	}
}
...
function __instance_deactivate_object(_inst) {
	...
	if (object_is_singleton(obj) || object_is_system(obj)) {
		...
		__INSTANCE_DEACTIVATE_OBJECT(_inst);
	}
}
...
function __instance_deactivate_region(_left, _top, _width, _height, _inside, _notme) {
	var me = id;
	with (all) {
...
function __instance_copy(_perf) {
	var error = __object_get_diagnostic(object_index);
	if (error != undefined) {
		show_error(error, false);
		return noone;
	}
	return __INSTANCE_COPY(_perf);
}

/// @desc Wrapper for `instance_destroy` which checks whether the instance is able to be safely destroyed.
/// @param {real} [id] The id of the instance to destroy.
/// @param {bool} [performevent] Whether to perform the instance destroy code.
function __instance_destroy() {
	...
	__INSTANCE_DESTROY(inst, perf);
}

for (var i = 0; object_exists(i); i += 1) {
	var is_singleton = object_is_singleton(i);
	var is_control = object_is_system(i);
	if (is_singleton || is_control) {
